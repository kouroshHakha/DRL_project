...
import statistics
import os
from framework.wrapper.TIAClass import * 
import IPython
import itertools
import pickle
...
class TIA(gym.Env):
    ...
    def reset(self):
        ...
        self.cur_params_idx = [1, 0, 3, 3, 7, 7] 
    ...
    def reward(self, spec, goal_spec):
        ...
        reward = 0.0
        for i,rel_spec in enumerate(rel_specs):
            if (self.specs_id[i] == 'ibias') or (self.specs_id[i] == 'tset'):
                rel_spec = rel_spec*-1.0
            if rel_spec < 0:
                reward += rel_spec
    ...
    def update(self, params_idx):
        ...
        cur_specs = np.array([ol_results[0][1]['f3db'], ol_results[0][1]['gain']/2, tset]) 
