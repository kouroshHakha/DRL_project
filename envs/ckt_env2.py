...
class CSAmp(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=np.full(2*len(self.specs_id), CSAmp.PERF_LOW),
                                            high=np.full(2*len(self.specs_id), CSAmp.PERF_HIGH))

        #initialize current param/spec observations
        self.cur_specs = np.zeros(self.observation_space.shape[0], dtype=np.int32)
        self.cur_params_idx = np.zeros(self.action_space.shape[0], dtype=np.int32)
        # _ = self.reset()
    ...
    def reset(self, z=None, sigma=0.2):
        ...
        return np.concatenate([np.ones(spec_size)*CSAmp.PERF_LOW, self.specs_ideal_norm], axis=0)
    ...
    def step(self, action):
        ...
        return np.concatenate([cur_spec, self.specs_ideal_norm]), rewards, done, {"params": action}
