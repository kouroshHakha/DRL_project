...
import statistics
import os
#TODO: import your simulation framework here
#ex: from framework.wrapper.TwoStageClass import TwoStageClass
import IPython
import itertools
import pickle
...
class ##TODO: name env here, ex: TwoStageAmp(gym.Env):
    ...
    #CIR_YAML = framework_path[0]+"/yaml_files/two_stage_opamp.yaml"
     ...
     def __init__(self, env_config):
        ...
        self.env_steps = 0
        with open(#TODO: point to yaml here, ex: TwoStageAmp.CIR_YAML, 'r') as f:
        ...
        dsn_netlist = #TODO: point to framework path, ex. TwoStageAmp.framework_path[0] + yaml_data['dsn_netlist']
        #TODO: point to your sim env
        #ex: self.sim_env = TwoStageClass(design_netlist=dsn_netlist)
        self.action_meaning = [-1,0,2]
        self.action_space = spaces.Tuple([spaces.Discrete(len(self.action_meaning))]*len(self.params_id))
        self.observation_space = spaces.Box(
            #TODO: add high low perf
    ...
    def reset(self):
        ...
        self.cur_params_idx = #add reset based on dimension of parameters #np.array([33, 33, 33, 33, 33, 33, 33])
...
#        self.cur_params_idx = self.cur_params_idx + np.array(self.action_arr[int(action)])
    ...
    def reward(self, spec, goal_spec):
        ...
        reward = 0.0
        for i,rel_spec in enumerate(rel_specs):
            if( #TODO: check that you want this here, ex.self.specs_id[i] == 'ibias_max'):
                rel_spec = rel_spec*-1.0
            if rel_spec < 0:
                reward += rel_spec
